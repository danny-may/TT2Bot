...
{
    class EquipmentEmbedable : GameEntitySingleEmbedable<Equipment>
    {
        public EquipmentEmbedable(ICommandContext context, Equipment entity, double? level) : base(context, entity, null, level) { }

        protected override ILocalisable<string> Title => new LocalisedString(EquipmentText.SHOW_FOOTER, BotUser.Username, Entity.FileVersion);
        protected override ILocalisable<string> Footer => new LocalisedString(EquipmentText.SHOW_TITLE, Entity.Name);
        ...
        protected override LocalisedEmbedBuilder GetBaseEmbed()
            => base.GetBaseEmbed().AddInlineField(f => f.WithName(EquipmentText.SHOW_FIELD_ID).WithRawValue(Entity.Id))
        ...
        protected override void AddFields(LocalisedEmbedBuilder builder)
        {
            if (To is double level)
                builder.AddField(f => f.WithName(EquipmentText.SHOW_FIELD_BONUSAT, level, level * 10).WithValue(Entity.BonusType.LocaliseValue(Entity.BonusOnLevel(10 * level))))
                       .AddField(f => f.WithName(TitanBot.TBLocalisation.NOTES).WithValue(EquipmentText.SHOW_FIELD_NOTE));
            else
                builder.AddInlineField(f => f.WithName(EquipmentText.SHOW_FIELD_BONUSBASE).WithValue(Entity.BonusType.LocaliseValue(Entity.BonusBase)))
                       .AddInlineField(f => f.WithName(EquipmentText.SHOW_FIELD_BONUSINCREASE).WithValue(Entity.BonusType.LocaliseValue(Entity.BonusIncrease)))
